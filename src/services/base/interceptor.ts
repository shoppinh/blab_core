...
const handleRefreshToken = async (accessToken: any, refreshToken: any, roleId: any) => {
  let tokenReturn;
  ...
  if (response.status === 406) {
    ...
    if (!isLogout) {
      ...
      if (response.status === 203) {
        toast.error(i18next.t('login.accountHasBeenDeactivated') as string);
      }
      store.dispatch(
        sessionActions.doLogout({
          token: sessionAuth?.accessToken || '',
